#!/usr/bin/env node
/**
 * FIELD-BY-FIELD CROPPED VISUAL PROOF — Before/After Screenshots
 *
 * For each field in a section:
 *   1. Navigate to section in PDF Layout mode (100% opacity, Show All)
 *   2. Locate the field element via data-field-key attribute
 *   3. Crop a tight screenshot around the field (with padding for context)
 *   4. Screenshot BEFORE setting value (baseline)
 *   5. Set the field's unique value via Jotai
 *   6. Screenshot AFTER (shows the value appeared at the correct position)
 *   7. Clear the field, move to next
 *
 * Produces paired cropped before/after screenshots proving each field maps correctly.
 *
 * Usage:
 *   cd app && node ../scripts/field-by-field-cropped.mjs
 *
 * Env options:
 ...
 *   PADDING=60                     — pixel padding around field crop (default: 60)
 */
...
const SCREENSHOT_DIR = join(__dirname, 'field-by-field-cropped-proof');
const HEADLESS = process.env.HEADLESS !== '0';
const MAX_FIELDS = process.env.MAX_FIELDS ? parseInt(process.env.MAX_FIELDS) : Infinity;
const PADDING = process.env.PADDING ? parseInt(process.env.PADDING) : 60;
...
console.log(`Will validate ${totalFields} fields across ${fieldsBySection.size} sections (cropped before/after per field)`);
...
// Auth
const TEST_EMAIL = `fbfc-${Date.now()}@test.dev`;
...
let submissionId;
{
  ...
  const p = (await b.newContext({ viewport: { width: 1400, height: 1000 } })).newPage();
  const page = await p;
...
// Per-section, per-field validation with CROPPED before/after screenshots
...
for (const section of sortedSections) {
  ...
  try {
    ...
    // Per-field: scroll → locate element → crop BEFORE → set → crop AFTER → clear
    for (const field of fields) {
      ...
      // For radio fields, use first widget's data-field-key
      const fieldKey = field.uiFieldType === 'radio' && field.radioWidgets
        ? `${field.semanticKey}-radio-0`
        : field.semanticKey;
      ...
      try {
        ...
        // Locate the field element via data-field-key
        const fieldEl = page.locator(`[data-field-key="${fieldKey}"]`);
        let box = null;
        try {
          // Scroll element into view first
          await fieldEl.scrollIntoViewIfNeeded({ timeout: 3000 });
          box = await fieldEl.boundingBox({ timeout: 3000 });
        } catch (_) {
          // Fallback: take viewport screenshot if element not found
        }

        // Calculate clip region with padding (or fallback to full viewport)
        let clipRegion = null;
        if (box) {
          ...
          clipRegion = {
            ...
            width: Math.min(box.width + PADDING * 2, (vp?.width || 1400) - Math.max(0, box.x - PADDING)),
            height: Math.min(box.height + PADDING * 2, (vp?.height || 1000) - Math.max(0, box.y - PADDING)),
          };
        }

        // BEFORE screenshot (field empty, cropped around the field)
        const beforePath = join(sectionDir, `${safeKey}_BEFORE.png`);
        if (clipRegion) {
          await page.screenshot({ path: beforePath, clip: clipRegion });
        } else {
        ...
        // AFTER screenshot (field filled, cropped around the field)
        const afterPath = join(sectionDir, `${safeKey}_AFTER.png`);
        if (clipRegion) {
          await page.screenshot({ path: afterPath, clip: clipRegion });
        } else {
        ...
        results.push({
          ...
          testValue: String(testVal), cropped: !!clipRegion,
          cropBox: clipRegion,
...
const report = {
  ...
  summary: { total: globalCount, passed, failed, croppedCount: results.filter(r => r.cropped).length },
  fields: results,
};
const reportPath = join(SCREENSHOT_DIR, 'field-by-field-cropped-report.json');
writeFileSync(reportPath, JSON.stringify(report, null, 2) + '\n');

console.log(`\n========================================`);
console.log(`FIELD-BY-FIELD CROPPED VALIDATION COMPLETE`);
console.log(`  Total:   ${globalCount}`);
console.log(`  Passed:  ${passed}`);
console.log(`  Failed:  ${failed}`);
console.log(`  Cropped: ${results.filter(r => r.cropped).length}`);
