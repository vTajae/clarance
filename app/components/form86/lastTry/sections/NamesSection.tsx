      ...
      return (
        <div className="mt-2">
          <input
            ...
            id={question.id}
            name={question.id}
            className={`${checkboxClasses} mr-2 align-middle`}
            checked={!!answer}
            onChange={(e) => handleInputChange(question.id, e.target.checked)}
          />
          <label htmlFor={question.id} className="text-gray-700 align-middle">
            {question.label}
          </label>
        </div>
      );
        ...
        <div className="flex items-center flex-col">
          ...
          <div className="relative max-w-sm w-full">
            <input
              ...
              id={question.id}
              name={question.id}
              className={`${baseInputClasses} pl-10`} // Adjust padding to accommodate the icon
              value={(answer as string) || ""}
              onChange={(e) => handleInputChange(question.id, e.target.value)}
