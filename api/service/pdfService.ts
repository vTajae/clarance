...
class PdfService {
  ...
  async setFieldValue(field: any, fieldValue: string) {
    switch (field.constructor) {
      ...
      case PDFDropdown:
        ...
        break;
      case PDFCheckBox:
        fieldValue.toLowerCase() === "yes" ? field.check() : field.uncheck();
        break;
      case PDFRadioGroup:
        field.select(fieldValue);
        break;
  ...
  async loadAndFillPdf(
    ...
    try {
      ...
      // Create a map to reduce repeated getField calls
      const fieldMap = new Map<string, any>();
      
      // First pass: populate the fieldMap
      for (const item of finalForm) {
        if (item.value && !fieldMap.has(item.name)) {
          const field = form.getField(item.name);
          if (field) {
            fieldMap.set(item.name, field);
          } else {
            console.log(`Field ${item.name} not found in the PDF.`);
          }
        }
      }
      
      // Second pass: set the field values in parallel (if possible)
      await Promise.all(
        finalForm.map(async (item) => {
          if (item.value && fieldMap.has(item.name)) {
            await this.setFieldValue(fieldMap.get(item.name), item.value as string);
          }
        })
      );
      ...
      fs.writeFile(outputPath, modifiedPdfBytes, (err) => {
        if (err) {
